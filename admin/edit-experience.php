...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['add_experience'])) {
            ...
            header('Location: edit-experience.php?success=Experience added successfully!');
            exit;
            ...
            header('Location: edit-experience.php?success=Experience updated successfully!');
            exit;
        } elseif (isset($_POST['delete_experience'])) {
            $stmt = $pdo->prepare('DELETE FROM experience WHERE id = ?');
            $stmt->execute([$_POST['id']]);
            header('Location: edit-experience.php?success=Experience deleted successfully!');
            exit;
        }
    }
...
<div class="main-content">
    ...
    <div class="item-list">
        ...
        <?php foreach ($experiences as $exp): ?>
            <div class="item">
                <form action="edit-experience.php" method="POST" style="flex-grow: 1; display: flex; align-items: center;">
                    <input type="hidden" name="id" value="<?php echo $exp['id']; ?>">
                    <input type="text" name="title" value="<?php echo htmlspecialchars($exp['title']); ?>" style="margin-right: 10px;">
                    <input type="text" name="institution" value="<?php echo htmlspecialchars($exp['institution']); ?>" style="margin-right: 10px;">
                    <input type="text" name="start_year" value="<?php echo htmlspecialchars($exp['start_year']); ?>" style="width: 60px; margin-right: 5px;"> -
                    <input type="text" name="end_year" value="<?php echo htmlspecialchars($exp['end_year']); ?>" style="width: 60px; margin-left: 5px;">
