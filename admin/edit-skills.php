...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['add_skill'])) {
            ...
            header('Location: edit-skills.php?success=Skill added successfully!');
            exit;
            ...
            header('Location: edit-skills.php?success=Skill updated successfully!');
            exit;
        } elseif (isset($_POST['delete_skill'])) {
            $stmt = $pdo->prepare('DELETE FROM skills WHERE id = ?');
            $stmt->execute([$_POST['id']]);
            header('Location: edit-skills.php?success=Skill deleted successfully!');
            exit;
        }
    }
...
<div class="main-content">
    ...
    <div class="item-list">
        ...
        <?php foreach ($skills as $skill): ?>
            <div class="item">
                <form action="edit-skills.php" method="POST" style="flex-grow: 1; display: flex; align-items: center;">
                    <input type="hidden" name="id" value="<?php echo $skill['id']; ?>">
                    <input type="text" name="skill_name" value="<?php echo htmlspecialchars($skill['skill_name']); ?>" required style="margin-right: 10px;">
                    <input type="number" name="level" min="1" max="5" value="<?php echo htmlspecialchars($skill['level']); ?>" required style="margin-right: 10px;">
